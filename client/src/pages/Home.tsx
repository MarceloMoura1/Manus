...
export function getShellWorkspaceLayout(active: RouteId) {
  ...
  // Chamados owns its mobile menu trigger so the shared topbar is not mounted.
  // This removes its 56px structural height on every desktop viewport.
  if (active === "tickets") {
    return {
      moduleTopbar: "none" as const,
...
type ShellTopbarBoundaryProps = {
  active: RouteId;
  mode: ReturnType<typeof getShellWorkspaceLayout>["moduleTopbar"];
  children: React.ReactNode;
};

export function ShellTopbarBoundary({ active, mode, children }: ShellTopbarBoundaryProps) {
  if (active === "tickets" || mode === "none") return null;

  return (
    <div className={mode === "mobile-only" ? "lg:hidden" : undefined} data-testid="module-topbar-shell">
      {children}
    </div>
  );
}
...
export function ConversationsPage({ attendanceLaunch, attendancePhone }: {
  ...
  const serverIndeterminateHistory = conversationMessageResult?.indeterminateHistory ?? [];
  const indeterminateConversationHistory = [
    ...(Array.isArray(serverIndeterminateHistory) ? serverIndeterminateHistory : []),
    ...conversationTimeline.indeterminateHistory,
  ];
    ...
    try {
      if (attachmentToSend) {
        await sendAttachmentMutation.mutateAsync({
          ...
          mimeType: attachmentToSend.mimeType,
          fileName: attachmentToSend.fileName,
          mediaSource: attachmentToSend.kind === 'audio' ? 'attachment' : undefined,
  ...
  return (
      <div data-testid="attendance-workspace" data-theme={theme} className="group relative flex h-full min-h-0 w-full min-w-0 max-w-full overflow-hidden bg-white data-[theme=dark]:border-slate-800 data-[theme=dark]:bg-slate-950 data-[theme=dark]:shadow-black/30 min-[900px]:rounded-2xl min-[900px]:border min-[900px]:border-slate-200 min-[900px]:shadow-xl">
      ...
      <div className={cn(
        ...
        <div className="border-b border-slate-100 bg-white px-4 py-3 group-data-[theme=dark]:border-slate-800 group-data-[theme=dark]:bg-slate-950">
          <div className="flex items-center gap-2.5" data-testid="attendance-header">
            ...
            <h2 className="truncate text-base font-bold tracking-tight text-slate-900 group-data-[theme=dark]:text-slate-100">Atendimento</h2>
          </div>
          ...
          {dateFilterOpen && (
            <div className="mt-2 space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3 group-data-[theme=dark]:border-slate-700 group-data-[theme=dark]:bg-slate-800">

              {/* Busca por nome/empresa/telefone */}
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-slate-600 group-data-[theme=dark]:text-slate-300">Buscar</label>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400 group-data-[theme=dark]:text-slate-500" />
                  <input
                    type="text"
                    placeholder="Nome, empresa ou telefone..."
                    value={searchTerm}
                    onChange={e => setSearchTerm(e.target.value)}
                    className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-8 pr-8 text-xs text-slate-700 transition-all placeholder:text-slate-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-indigo-400 group-data-[theme=dark]:border-slate-700 group-data-[theme=dark]:bg-slate-900 group-data-[theme=dark]:text-slate-100 group-data-[theme=dark]:placeholder:text-slate-500"
                  />
                  {searchTerm && (
                    <button
                      onClick={() => setSearchTerm('')}
                      className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 group-data-[theme=dark]:text-slate-500 group-data-[theme=dark]:hover:text-slate-200"
                    >
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400 group-data-[theme=dark]:border-slate-700 group-data-[theme=dark]:bg-slate-900 group-data-[theme=dark]:text-slate-100"
                    />
                  </div>
                  <div className="flex-1">
              ...
              {hasListFilters && (
                <button
                  ...
                  className="w-full py-1 text-center text-xs font-medium text-indigo-600 underline hover:text-indigo-800 group-data-[theme=dark]:text-indigo-300 group-data-[theme=dark]:hover:text-indigo-200"
                >Limpar todos os filtros</button>
              )}

              {(hasDateFilter || searchTerm) && (
                <p className="text-center text-xs text-slate-500 group-data-[theme=dark]:text-slate-400">
        ...
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3 group-data-[theme=dark]:border-slate-800 group-data-[theme=dark]:bg-slate-950" data-testid="conversation-list-heading">
          ...
          <div className="flex shrink-0 items-center gap-1">
            <button
              ...
              className="flex h-8 items-center gap-1 rounded-md px-1.5 text-slate-600 transition-colors hover:bg-violet-50 hover:text-violet-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 group-data-[theme=dark]:text-slate-300 group-data-[theme=dark]:hover:bg-violet-500/20 group-data-[theme=dark]:hover:text-violet-200"
            >
        ...
        <div className="flex-1 overflow-y-auto">
          {conversationsLoading ? (
            <div className="flex h-48 items-center justify-center text-sm text-slate-500 group-data-[theme=dark]:text-slate-400" role="status">Carregando conversas...</div>
            ...
            <div className="flex flex-col items-center justify-center h-48 text-center px-6">
              <MessageCircle className="mb-3 h-10 w-10 text-slate-200 group-data-[theme=dark]:text-slate-700" />
              <p className="text-sm font-medium text-slate-500 group-data-[theme=dark]:text-slate-300">Nenhuma conversa</p>
              <p className="mt-1 text-xs text-slate-400 group-data-[theme=dark]:text-slate-500">Tente outro filtro ou busca</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={cn(
        ...
        {crmHandoffState === 'composer' && crmIntent && crmCustomerQuery.data?.client ? (
          ...
          <>
            {/* Header do Chat */}
            <div data-testid="active-conversation-header" className="flex min-w-0 flex-shrink-0 items-center justify-between gap-2 border-b border-slate-200/80 bg-white px-3 py-2.5 sm:px-4 md:px-6 md:py-3 group-data-[theme=dark]:border-slate-800 group-data-[theme=dark]:bg-slate-900">
              <div className="flex min-w-0 items-center gap-2 md:gap-3">
                <button
                  type="button"
                  onClick={returnToConversationList}
                  aria-label="Voltar para conversas"
                  className="flex h-11 flex-shrink-0 items-center gap-1 rounded-xl px-2 text-sm font-semibold text-blue-700 transition-colors hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 group-data-[theme=dark]:text-blue-300 group-data-[theme=dark]:hover:bg-blue-500/20 min-[900px]:hidden"
                >
                ...
                <div className="min-w-0">
                  <div className="flex min-w-0 items-center gap-1.5 md:gap-2">
                    <h3 className="truncate text-sm font-bold text-slate-900 group-data-[theme=dark]:text-slate-100 md:text-base">{selectedConv.name}</h3>
                    <span className={cn(
                      'hidden flex-shrink-0 rounded-full px-2 py-0.5 text-xs font-medium sm:inline',
                      selectedConv.status === 'open' ? 'bg-emerald-100 text-emerald-700 group-data-[theme=dark]:bg-emerald-500/20 group-data-[theme=dark]:text-emerald-300' :
                      selectedConv.status === 'bot' ? 'bg-violet-100 text-violet-700 group-data-[theme=dark]:bg-violet-500/20 group-data-[theme=dark]:text-violet-200' :
                      'bg-slate-100 text-slate-600 group-data-[theme=dark]:bg-slate-800 group-data-[theme=dark]:text-slate-300'
                    )}>
                      {selectedConv.status === 'open' ? 'Aberta' : selectedConv.status === 'bot' ? 'BOT' : 'Fechada'}
                    </span>
                  </div>
                  <p className="truncate text-xs text-slate-500 group-data-[theme=dark]:text-slate-400">{selectedConv.phone} {selectedConv.company ? `• ${selectedConv.company}` : ''}</p>
                </div>
              </div>
              <div className="flex flex-shrink-0 items-center gap-1.5 sm:gap-2">
                ...
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 shadow-sm transition-colors hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 group-data-[theme=dark]:border-slate-700 group-data-[theme=dark]:bg-slate-800 group-data-[theme=dark]:text-slate-300 group-data-[theme=dark]:hover:border-violet-500/40 group-data-[theme=dark]:hover:bg-violet-500/20 group-data-[theme=dark]:hover:text-violet-200"
                >
                ...
                <button type="button" aria-expanded={conversationDetailsOpen} aria-controls="conversation-details-panel"
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-600 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 group-data-[theme=dark]:text-slate-300 group-data-[theme=dark]:hover:bg-slate-800">
            ...
            {/* Input de Mensagem */}
            <div data-testid="conversation-composer" className="max-w-full flex-shrink-0 border-t border-slate-100 bg-white px-3 pt-3 group-data-[theme=dark]:border-slate-800 group-data-[theme=dark]:bg-slate-900 md:px-6 md:pt-4" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
              ...
              {replyTarget && (
                <div data-testid="conversation-composer-quote" className="mb-3 flex items-center gap-3 rounded-xl border border-violet-200 bg-violet-50 p-3 text-slate-700 group-data-[theme=dark]:border-violet-500/40 group-data-[theme=dark]:bg-violet-500/15 group-data-[theme=dark]:text-slate-200">
                  <Reply className="h-4 w-4 shrink-0 text-violet-700 group-data-[theme=dark]:text-violet-300" aria-hidden="true" />
                  <div className="min-w-0 flex-1 border-l-2 border-violet-400 pl-2.5">
                    <p className="truncate text-xs font-bold text-violet-800 group-data-[theme=dark]:text-violet-200">Respondendo a {replyAuthor(replyTarget)}</p>
                    <p className="truncate text-xs text-slate-600 group-data-[theme=dark]:text-slate-300">{replyPreview(replyTarget)}</p>
                  </div>
                  <button type="button" onClick={() => setReplyTarget(null)} aria-label="Cancelar resposta" title="Cancelar resposta" className="rounded-full p-1 text-slate-500 hover:bg-white hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 group-data-[theme=dark]:text-slate-400 group-data-[theme=dark]:hover:bg-slate-800 group-data-[theme=dark]:hover:text-slate-100"><X className="h-4 w-4" /></button>
                </div>
              )}
              {attachment && (
                <div className="mb-3 flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 p-3">
                  ...
                  <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-slate-700 group-data-[theme=dark]:text-slate-100">{attachment.fileName}</p><p className="text-xs text-slate-500 group-data-[theme=dark]:text-slate-400">Adicione uma legenda e pressione Enter para enviar</p></div>
                  <button type="button" onClick={() => setAttachment(null)} className="rounded-full p-1 text-slate-500 hover:bg-white group-data-[theme=dark]:text-slate-400 group-data-[theme=dark]:hover:bg-slate-800"><X className="h-4 w-4" /></button>
                </div>
              )}
              <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap sm:gap-3">
                ...
                <div className="relative min-w-0 flex-[1_1_10rem]">
                  <input
                    ...
                    className={cn(
                       'w-full rounded-2xl border bg-slate-50 px-4 py-3 pr-12 text-sm transition-all placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 group-data-[theme=dark]:bg-slate-800 group-data-[theme=dark]:text-slate-100 group-data-[theme=dark]:placeholder:text-slate-500 group-data-[theme=dark]:focus:bg-slate-800',
                       waConnected === false ? 'border-red-200 bg-red-50 cursor-not-allowed opacity-60 group-data-[theme=dark]:border-red-500/40 group-data-[theme=dark]:bg-red-500/15' : 'border-slate-200 group-data-[theme=dark]:border-slate-700'
                    )}
                  />
                </div>
                <button
              ...
              <div className="flex items-center justify-between mt-2 ml-1">
                <p className="text-xs text-slate-400 group-data-[theme=dark]:text-slate-500">Enviando como <span className="font-medium text-slate-600 group-data-[theme=dark]:text-slate-300">{userName}</span></p>
                <div className="flex items-center gap-1.5">
                  <span className={cn('w-2 h-2 rounded-full', waConnected === null ? 'bg-yellow-400' : waConnected ? 'bg-green-400' : 'bg-red-500')}></span>
                  <span className="text-xs text-slate-400 group-data-[theme=dark]:text-slate-500">{waConnected === null ? 'Verificando...' : waConnected ? 'WhatsApp conectado' : 'WhatsApp desconectado'}</span>
                </div>
              </div>
            </div>
          </>
        ) : (
          <div data-testid="attendance-empty-state" className="flex flex-1 flex-col items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 group-data-[theme=dark]:from-slate-950 group-data-[theme=dark]:to-slate-900">
            <div className="text-center">
              <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-blue-100 to-violet-100 group-data-[theme=dark]:from-blue-500/20 group-data-[theme=dark]:to-violet-500/20">
                <MessageCircle className="w-10 h-10 text-blue-400" />
              </div>
              <h3 className="mb-2 text-xl font-bold text-slate-700 group-data-[theme=dark]:text-slate-100">Selecione uma conversa</h3>
              <p className="max-w-xs text-sm text-slate-400 group-data-[theme=dark]:text-slate-500">Escolha uma conversa da lista à esquerda para começar a atender</p>
            </div>
          </div>
        )}
      </div>
      ...
      {transferOpen && selectedConv && (
        <div data-testid="transfer-conversation-dialog" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/30 p-4" role="dialog" aria-modal="true" aria-labelledby="transfer-conversation-title" onMouseDown={(event) => {
          ...
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 shadow-xl group-data-[theme=dark]:border-slate-700 group-data-[theme=dark]:bg-slate-900 group-data-[theme=dark]:shadow-black/40 sm:p-5">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="min-w-0"><h3 id="transfer-conversation-title" className="font-semibold text-slate-900 group-data-[theme=dark]:text-slate-100">Transferir conversa</h3>{transferConversationMutation.isPending && <span role="status" className="mt-0.5 block text-xs text-violet-700 group-data-[theme=dark]:text-violet-300">Transferindo atendimento…</span>}</div>
              <button type="button" onClick={() => setTransferOpen(false)} disabled={transferConversationMutation.isPending} aria-label="Fechar transferência" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 disabled:cursor-not-allowed disabled:opacity-50 group-data-[theme=dark]:text-slate-400 group-data-[theme=dark]:hover:bg-slate-800"><X className="h-4 w-4" /></button>
            </div>
            <div className="max-h-64 space-y-1 overflow-y-auto pr-1">
              {(activeUsersData ?? []).map((user) => (
                ...
                })} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 disabled:cursor-not-allowed disabled:opacity-50 group-data-[theme=dark]:hover:bg-violet-500/20">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-violet-100 text-xs font-bold text-violet-700 group-data-[theme=dark]:bg-violet-500/20 group-data-[theme=dark]:text-violet-200">{getInitials(user.name)}</span>
                  <span className="min-w-0 flex-1 truncate text-sm font-medium text-slate-800 group-data-[theme=dark]:text-slate-200">{user.name}</span>
                  {user.id === selectedConv.assignedUserId && <span className="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500 group-data-[theme=dark]:bg-slate-800 group-data-[theme=dark]:text-slate-300">Atual</span>}
                </button>
              ))}
            </div>
          </div>
      ...
      {closeConfirmOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
          <div className="w-80 rounded-lg bg-white p-4 shadow-lg pointer-events-auto group-data-[theme=dark]:bg-slate-900 group-data-[theme=dark]:shadow-black/40">
            <p className="mb-4 text-center font-semibold text-slate-900 group-data-[theme=dark]:text-slate-100">Encerrar conversa?</p>
            <div className="flex gap-2">
              <button
                onClick={() => setCloseConfirmOpen(false)}
                className="flex-1 rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium text-slate-900 transition-colors hover:bg-slate-200 group-data-[theme=dark]:bg-slate-800 group-data-[theme=dark]:text-slate-100 group-data-[theme=dark]:hover:bg-slate-700"
              >
                Não
              </button>
              <button
      ...
      {reopenConfirmOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
          <div className="w-80 rounded-lg bg-white p-4 shadow-lg pointer-events-auto group-data-[theme=dark]:bg-slate-900 group-data-[theme=dark]:shadow-black/40">
            <p className="mb-4 text-center font-semibold text-slate-900 group-data-[theme=dark]:text-slate-100">Reabrir conversa?</p>
            <div className="flex gap-2">
              <button
...
export function TicketsPage({ onOpenMobileMenu }: { onOpenMobileMenu?: () => void }) {
  ...
  const handleTicketAttachmentUpload = async () => {
    ...
    try {
      ...
      await utils.chamados.getDetail.invalidate({ chamadoId: selectedChamado.id });
    } catch (error) {
  ...
  return (
    <div className={`flex h-screen h-[100dvh] min-w-0 overflow-hidden bg-slate-50 ${theme === 'dark' ? 'dark bg-slate-950' : ''}`}>
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <ShellTopbarBoundary active={active} mode={shellLayout.moduleTopbar}>
            <ModuleTopbar
