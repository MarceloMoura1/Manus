...
export function NewAttendanceFlow({ onNavigate, initialPhone, initialCrmCustomer, embedded = false, onCancel }: {
  ...
  const handleCreateCustomer = async (event: FormEvent) => {
    ...
    try {
      ...
      await utils.megadesk.attendanceRecipient.invalidate();
    } catch (createError) {
  ...
  const handleSend = async () => {
    if (!recipient || !message.trim() || isSubmitting) return;
    if (knownActive) {
  ...
  return (
    <section className={cn('flex min-h-0 h-full flex-col bg-slate-50 text-slate-900', embedded ? 'p-3 sm:p-5' : 'min-h-screen p-4 sm:p-8')} data-testid="new-attendance-flow">
      <div className="mx-auto flex h-full w-full max-w-3xl min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
          ...
          {recipient && <div className="mt-5 space-y-4">
            ...
            {knownActive && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4" role="status" data-testid="active-attendance-warning"><div className="flex items-start gap-3"><AlertCircle className="mt-0.5 h-5 w-5 flex-shrink-0 text-amber-700" /><div><h3 className="font-semibold text-amber-950">Atendimento já em andamento</h3><p className="mt-1 text-sm text-amber-900">Já existe um atendimento ativo para este número. Uma segunda conversa não será criada.</p></div></div><button type="button" onClick={openExistingConversation} className="mt-4 rounded-xl bg-amber-700 px-3 py-2 text-sm font-semibold text-white transition hover:bg-amber-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500">Abrir atendimento</button></div>}
          </div>}
        </div>
        ...
        <div className="border-t border-slate-100 bg-white p-3 sm:p-4" data-testid="new-attendance-message-composer">
          ...
          <div className="flex items-end gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100"><textarea id="attendance-message" value={message} disabled={!recipient || !!knownActive || isSubmitting} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void handleSend(); } }} placeholder={recipient ? 'Digite uma mensagem...' : 'Selecione um destinatário para escrever...'} rows={1} className="max-h-32 min-h-11 flex-1 resize-y bg-transparent px-2 py-2 text-sm outline-none placeholder:text-slate-400 disabled:cursor-not-allowed" /><button type="button" onClick={handleSend} disabled={!recipient || !!knownActive || !message.trim() || isSubmitting} aria-label="Enviar mensagem" className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-blue-600 text-white transition hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 disabled:cursor-not-allowed disabled:bg-slate-300">{isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</button></div>
