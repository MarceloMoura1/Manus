...
export type ErpSection = "summary" | "clients" | "products" | "stock" | "suppliers" | "purchases" | "sales" | "finance" | "fiscal";
...
export function ERPWorkspace({ section, onNavigate, canAccessClients, canAccessFinance, canAccessFiscal, initialCrmClientId, onClientNavigate }: { section: ErpSection; onNavigate: (section: ErpSection) => void; canAccessClients: boolean; canAccessFinance: boolean; canAccessFiscal: boolean; initialCrmClientId?: string; onClientNavigate: (phone: string) => void }) {
  useErpRealtime();
  ...
  const content = section === "summary" ? <Summary onNavigate={onNavigate}/> : section === "clients" ? canAccessClients ? <ClientesPage initialSelectedId={initialCrmClientId} onNavigate={onClientNavigate}/> : denied : section === "products" ? <Products/> : section === "suppliers" ? <SuppliersPage/> : section === "purchases" ? <PurchasesPage/> : section === "sales" ? <SalesPage/> : section === "finance" ? canAccessFinance ? <FinancePage/> : denied : section === "fiscal" ? canAccessFiscal ? <FiscalPage/> : denied : <Stock/>;
  const available: Array<{ id: ErpSection; label: string }> = [{id:"summary",label:"Resumo"},...(canAccessClients?[{id:"clients" as const,label:"Clientes"}]:[]),{id:"products",label:"Produtos"},{id:"stock",label:"Estoque"},{id:"suppliers",label:"Fornecedores"},{id:"purchases",label:"Compras"},{id:"sales",label:"Vendas"},...(canAccessFinance?[{id:"finance" as const,label:"Financeiro"}]:[]),...(canAccessFiscal?[{id:"fiscal" as const,label:"Fiscal"}]:[])];
  const planned = ["Relatórios","Integrações"];
...
function Summary({ onNavigate }: { onNavigate: (section: ErpSection) => void }) {
  ...
  return <div className="space-y-6" data-testid="erp-summary-page">
    <header><h1 className="text-2xl font-bold text-slate-900">Resumo do ERP</h1><p className="text-sm text-slate-600">Indicadores calculados pelo backend para o tenant atual.</p></header>
...
function Products() {
  ...
  const create = trpc.erp.products.create.useMutation({ onSuccess: () => done("Produto cadastrado com sucesso.") });
  const update = trpc.erp.products.update.useMutation({ onSuccess: () => done("Produto atualizado com sucesso.") });
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); if(!form||pending) return; const command={ name:form.name,sku:form.sku,barcode:form.barcode||undefined,description:form.description||undefined,category:form.category||undefined,unit:form.unit,costPriceCents:cents(form.cost),salePriceCents:cents(form.sale),minimumStock:form.minimumStock }; if(command.costPriceCents<0||command.salePriceCents<0){setMessage("Informe valores monetários válidos.");return;} if(form.publicId) update.mutate({...command,publicId:form.publicId}); else create.mutate(command); };
  const edit = (product: NonNullable<typeof query.data>["items"][number]) => setForm({ publicId:product.publicId,name:product.name,sku:product.sku,barcode:product.barcode??"",category:product.category??"",unit:product.unit,cost:(product.costPriceCents/100).toFixed(2).replace(".",","),sale:(product.salePriceCents/100).toFixed(2).replace(".",","),minimumStock:product.minimumStock,description:product.description??"" });
...
function Stock() {
  ...
  return <div className="space-y-5" data-testid="erp-stock-page">
    <header className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-2xl font-bold">Estoque</h1><p className="text-sm text-slate-600">Saldo atual e ledger imutável.</p></div>{canWrite&&<Button onClick={()=>setForm({productPublicId:"",type:"manual_in",quantity:"",reason:"",idempotencyKey:newKey()})}>Nova movimentação</Button>}</header>
