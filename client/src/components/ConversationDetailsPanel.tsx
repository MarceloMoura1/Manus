...
export function ConversationDetailsPanel({ conversation, open, canManageClients, onClose, onContactUpdated, onCrmLinked, onNavigate, onToast }:
  ...
  const historyIndeterminate = historyComposition.indeterminateHistory;
  ...
  const historyModal = historyId && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/40 p-0 sm:p-4" onMouseDown={event => { if (event.currentTarget === event.target) setHistoryId(null); }}>
    <div role="dialog" aria-modal="true" aria-labelledby="history-title" className="flex h-full w-full flex-col overflow-hidden bg-white sm:h-[88vh] sm:max-w-5xl sm:rounded-2xl">
      <header className="flex justify-between border-b p-4"><div><span className="text-xs font-semibold text-blue-700">Somente leitura</span><h2 id="history-title" className="text-lg font-semibold">{historyDetail.data?.conversation.customerName || "Conversa anterior"}</h2><p className="text-xs">{historyDetail.data?.conversation.publicCode} · {historyDetail.data?.conversation.status} · {dateTime(historyDetail.data?.conversation.startedAt)}</p><p className="text-xs">Responsável: {historyDetail.data?.conversation.assignedUserName || "Não atribuído"}</p></div><button autoFocus aria-label="Fechar histórico" onClick={() => setHistoryId(null)}><X /></button></header>
      <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-4">
        <QueryState loading={historyDetail.isLoading} error={historyDetail.isError} empty={!historyTimeline.length && !historyIndeterminate.length} emptyText="Nenhuma atividade ou mensagem.">
          <div className="space-y-3">
            {historyTimeline.map((item: any, index: number) => {
              if (item.kind === "activity") return <ConversationActivityEvent key={item.id || index} event={item} compact />;
              const outbound = item.direction === "outbound" || item.from === "agent";
              const senderName = outbound ? operatorDisplayName(item) : item.sender;
              return <div key={item.id || index} className={cn("flex", outbound ? "justify-end" : "justify-start")}><div className={cn("max-w-[85%] rounded-2xl p-3", outbound ? "bg-blue-600 text-white" : "bg-slate-100")}>{item.replyTo && <ReadonlyQuote replyTo={item.replyTo} />}{item.type === "text" || !item.type ? <p>{item.text}</p> : <ConversationMedia conversationId={historyId} message={item} fallback={<span>{item.text || "Mídia indisponível"}</span>} />}<span className="block text-[10px] opacity-70">{senderName} · {dateTime(item.timestamp)}</span></div></div>;
            })}
            {historyIndeterminate.length > 0 && <section aria-label="Histórico anterior sem horário confirmado" className="space-y-3 border-t border-dashed border-slate-300 pt-4">
              <h3 className="text-center text-xs font-semibold text-slate-500">Histórico anterior sem horário confirmado</h3>
              {historyIndeterminate.map((item: any, index: number) => {
                if (item.kind === "activity") return <ConversationActivityEvent key={item.id || `history-indeterminate-activity-${index}`} event={item} compact />;
                const outbound = item.direction === "outbound" || item.from === "agent";
                return <div key={item.id || `history-indeterminate-${index}`} className={cn("flex", outbound ? "justify-end" : "justify-start")}><div className={cn("max-w-[85%] rounded-2xl p-3", outbound ? "bg-slate-600 text-white" : "bg-slate-100")}>{item.type === "text" || !item.type ? <p>{item.text || item.message}</p> : <ConversationMedia conversationId={historyId} message={item} fallback={<span>{item.text || item.message || "Mídia indisponível"}</span>} />}</div></div>;
              })}
            </section>}
          </div>
        </QueryState>
      </div>
      <footer className="border-t p-3 text-right"><button onClick={() => setHistoryId(null)} className="rounded bg-slate-900 px-4 py-2 text-white">Voltar para o atendimento</button></footer>
    </div>
  </div>;
