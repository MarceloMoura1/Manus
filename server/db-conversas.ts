...
export async function addMessageToConversation(
  ...
  if (!['customer', 'agent', 'bot'].includes(from)) {
    throw new Error('from deve ser customer, agent ou bot');
  }

  const sanitizedText = sanitizeString(text, MAX_MESSAGE_LENGTH);
  ...
  return retryWithBackoff(async () => {
    ...
    console.log(`[LOG] Adicionando mensagem à conversa ${conversationId}`);
    ...
    console.log(`[SUCCESS] Mensagem adicionada à conversa ${conversationId}`);
  });
}
