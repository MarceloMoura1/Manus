...
function publicProduct(row: ProductRow) { return { publicId: row.public_id, name: row.name, sku: row.sku, barcode: row.barcode, description: row.description, category: row.category, unit: row.unit, costPriceCents: Number(row.cost_price_cents), salePriceCents: Number(row.sale_price_cents), minimumStock: row.minimum_stock, active: row.active === 1, quantity: row.quantity, createdAt: row.created_at, updatedAt: row.updated_at }; }
